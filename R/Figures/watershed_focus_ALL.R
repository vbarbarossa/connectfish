...
for(i in 1:length(basin_names)){
  ...
  for(cat_fish in c('Non diadromous','Diadromous')){
    ...
    p <- annotate_figure(ggarrange(ggplotGrob(p_base),ggplotGrob(p_maps[[1]]),ggplotGrob(p_maps[[2]]),ggplotGrob(p_bp),
                                   nrow=1,ncol = 4,widths = c(0.5,0.5,0.5,0.4)),
                         left = text_grob(paste0(ws_lab,' (',length(unique(ci$binomial)),')'),rot = 90))
    
    # return(p)
    if(cat_fish == 'Non diadromous') ln[[i]] <- p
    if(cat_fish == 'Diadromous') ld[[i]] <- p
    ...
    if(i == 1){
      ...
      p <- annotate_figure(ggarrange(cowplot::get_legend(p_leg_dams),cowplot::get_legend(p_leg_ci),NULL,
                                     nrow=1,ncol = 3,widths = c(0.5,1,0.4))
                           ,left = text_grob("Am", color = "white",rot = 90))
...
# Non-diad
# Amazon, Congo, Mekong, Niger
ptot <- ggarrange(ph,ln[[1]],ln[[2]],ln[[3]],ln[[4]],pl,nrow=6,ncol = 1,heights = c(0.16,1,1,1,1,0.2))
ggsave(filename = paste0(dir_('figs/Figure_3/'),'nondiad.jpg'),ptot,
       width = 200*0.87,height = (8+10+50*4)*0.87,units = 'mm',dpi = 600,type='cairo')
ggsave(filename = paste0(dir_('figs/Figure_3/'),'nondiad.pdf'),ptot,
       width = 200*0.87,height = (8+10+50*4)*0.87,units = 'mm')


# Diad
# Salween, Danube, Comoe, Purari
ptot <- ggarrange(ph,ld[[7]],ld[[6]],ld[[5]],ld[[8]],pl,nrow=6,ncol = 1,heights = c(0.16,1,1,1,1,0.2))
ggsave(filename = paste0(dir_('figs/Figure_3/'),'diad.jpg'),ptot,
       width = 200*0.87,height = (8+10+50*4)*0.87,units = 'mm',dpi = 600,type='cairo')
ggsave(filename = paste0(dir_('figs/Figure_3/'),'diad.pdf'),ptot,
       width = 200*0.87,height = (8+10+50*4)*0.87,units = 'mm')
