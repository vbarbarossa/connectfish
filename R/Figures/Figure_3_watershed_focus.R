...
library(sf); library(foreach); library(data.table); library(viridis); library(ggplot2); library(ggpubr)

# computed CI for current and future for each species-basin (data.frame) #####################################
CI_tab <- readRDS('proc/CI_tab_global.rds')
...
iucn <- iucn[!duplicated(iucn$binomial),]
...
CI_stats <- as.data.table(readRDS('proc/CI_stats_HYBAS_ID.rds'))
