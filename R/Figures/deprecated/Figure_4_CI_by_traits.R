...
CI_tab <- rbind(readRDS('proc/CI_tab_global.rds'),readRDS('proc/CI_tab_global.rds'))

# compute the mean across each watershed
tab <- do.call('rbind',
               lapply(
                 split(CI_tab,CI_tab$binomial),function(x) data.frame(
                   binomial = unique(as.character(x$binomial)),
                   value = c(sum(x$connectivity.cur*x$patches.cum.area)/sum(x$patches.cum.area),
                             sum(x$connectivity.fut*x$patches.cum.area)/sum(x$patches.cum.area)),
                   variable = c('Current CI','Future CI'),
                   area = sum(x$patches.cum.area)
...
iucn <- foreach(i = 1:2,.combine = 'rbind') %do% foreign::read.dbf(paste0('/vol/milkunarc/vbarbarossa/data/IUCN/FW_FISH_20181113/FW_FISH_PART_',i,'.dbf'))
iucn_simplified <- droplevels(do.call('rbind',lapply(split(iucn,iucn$binomial),function(x) x[1,])))
