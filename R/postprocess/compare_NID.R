...
for(oth in c(10)){
  ...
  p <- ggplot(compare_na_nid) +
    ...
    xlab('CI (NID+GRanD+GOODD) [%]') +
    ylab('CI (GRanD+GOODD) [%]') +
    coord_cartesian(expand=F) +
    facet_wrap('type',ncol = 2) +
    theme_bw()
  ggsave(paste0(dir_figures_NID,'compare_NID_oth',oth,'.jpg'),p,width = 200, height = 100,units = 'mm',type='cairo')
  ...
  p <- ggplot(compare_na_nid %>% reshape2::melt(data=.,id.vars = c('binomial','type')) %>% as_tibble()) +
    ...
    theme_bw()
  ggsave(paste0(dir_figures_NID,'compare_NID_oth',oth,'_boxplot.jpg'),p,width = 200, height = 100,units = 'mm',type='cairo')
  
  if(oth == 10) saveRDS(compare_na_nid %>% reshape2::melt(data=.,id.vars = c('binomial','type')),'proc/compare_NID.rds')
