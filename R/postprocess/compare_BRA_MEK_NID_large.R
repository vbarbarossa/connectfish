...
for(reg in c('BRA','MEK','NID')){
  
  #------------------------------------------------------------
  #> OPTIONS
  file_GG <- paste0('proc/CI_tab_',reg,'_GGonly.rds')
  file_NID <- paste0('proc/CI_tab_',reg,'_large.rds')
  if(reg == 'NID') file_NID <- paste0('proc/CI_tab_',reg,'_large2.rds')
  ...
  saveRDS(compare_na_nid %>% reshape2::melt(data=.,id.vars = c('binomial','type')),paste0('proc/compare_',reg,'_large.rds'))
